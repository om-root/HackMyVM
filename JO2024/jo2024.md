...
```
Two open ports: `22/tcp` (OpenSSH 9.2p1 Debian) and <br>`80/tcp` (Apache 2.4.61, hosting a "Paris 2024 Olympic Games" themed site).
...
- While enumerating from the `www-data` shell, discovered a `.Xauthority` file sitting in a `/backup` directory.
