...
Exploitation
Uploaded a PHP reverse shell renamed to `shell.php.gif` to bypass the extension filter.
Since the server saves the file with a ROT13'd name, requested the ROT13 equivalent of the filename (e.g. `cuceri.cuc.gif` for `phprev.php.gif`) to trigger execution.
Caught a reverse shell with a `nc` listener on port 4444.
Grabbed `user.txt`.
...
Always audit `sudo -l` output against GTFOBins.
